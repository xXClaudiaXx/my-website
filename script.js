...
async function scanSymbols() {
  ...
  try {
    ...
    try {
      ...
      if (!proxyRes.ok) throw directErr;
      data = await proxyRes.json();
      scanStatusEl.textContent = 'Yahoo 直連失敗，已改用代理抓取。';
    }
    ...
    if (!list.length) scanStatusEl.textContent = '查無可用資料，請確認代號（例如 2330,2317）。';
  } catch (err) {
    const hint = location.protocol === 'file:'
      ? '你目前是用 file:// 開啟，請改用 python3 -m http.server 後開 http://localhost:8080'
      : '可能是 Yahoo CORS/網路限制，請稍後重試或改用可用代理。';
    scanStatusEl.textContent = `抓取失敗：${err.message}。${hint}`;
  }
}
